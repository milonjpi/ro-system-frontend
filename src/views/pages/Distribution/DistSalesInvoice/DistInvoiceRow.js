...
import DistInvoiceView from './DistInvoiceView';
import UpdateDistInvoice from './UpdateDistInvoice';
import { useDeleteDistInvoiceMutation } from 'store/api/distInvoice/distInvoiceApi';

const DistInvoiceRow = ({ sn, data }) => {
  const [open, setOpen] = useState(false);
  const [dialog, setDialog] = useState(false);
  const [view, setView] = useState(false);

  const dispatch = useDispatch();
  const [deleteDistInvoice] = useDeleteDistInvoiceMutation();

  const handleDelete = async () => {
    setDialog(false);
    try {
      const res = await deleteDistInvoice(data?.id).unwrap();
      if (res.success) {
        dispatch(
          setToast({
            open: true,
  ...
  return (
    <StyledTableRow>
      ...
      <StyledTableCell>
        {data?.distInvoicedProducts?.map((el) => (
