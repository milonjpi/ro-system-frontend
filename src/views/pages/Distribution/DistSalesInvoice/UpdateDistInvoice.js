...
import DistProductFields from './DistProductFields';
import { useDistClientDetailsQuery } from 'store/api/distClient/distClientApi';
import { useUpdateDistInvoiceMutation } from 'store/api/distInvoice/distInvoiceApi';
...
const UpdateDistInvoice = ({ open, handleClose, preData }) => {
  ...
  const { register, handleSubmit, control } = useForm({
    defaultValues: {
      products: preData?.distInvoicedProducts || [productValue],
    },
  });
  ...
  const [updateDistInvoice] = useUpdateDistInvoiceMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await updateDistInvoice({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
...
export default UpdateDistInvoice;
