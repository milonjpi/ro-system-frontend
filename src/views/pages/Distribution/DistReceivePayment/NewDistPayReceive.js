...
import { useDistReceivePaymentMutation } from 'store/api/distVoucher/distVoucherApi';
...
const NewDistPayReceive = ({ open, handleClose }) => {
  ...
  const handleCustomer = (value) => {
    setCustomer(value);
    if (value) {
      reset({ invoices: value.distInvoices || [] });
    } else {
      reset({ invoices: [] });
    }
  };
  ...
  const [distReceivePayment] = useDistReceivePaymentMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await distReceivePayment({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        setDate(moment());
...
export default NewDistPayReceive;
