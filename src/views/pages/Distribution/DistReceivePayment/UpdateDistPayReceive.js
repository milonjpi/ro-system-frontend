...
import { useUpdateDistReceivePaymentMutation } from 'store/api/distVoucher/distVoucherApi';
import DistInvoicesRow from './DistInvoicesRow';
...
const UpdateDistPayReceive = ({ open, handleClose, preData }) => {
  const { customer: preCustomer, distVoucherDetails, ...othersData } = preData;
  // library
  ...
  const voucheredInvoice = distVoucherDetails?.map((el) => ({
    ...el.invoice,
  ...
  const combinedInvoice = uniqueArray(
    voucheredInvoice || [],
    findCurrentCustomer?.distInvoices || [],
    'id'
  );
  // end library
  ...
  const [updateDistReceivePayment] = useUpdateDistReceivePaymentMutation();
  ...
  const onSubmit = async (data) => {
    ...
    const newData = {
      ...
      distVoucherDetails: data?.invoices
    ...
    try {
      setLoading(true);
      const res = await updateDistReceivePayment({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
...
export default UpdateDistPayReceive;
