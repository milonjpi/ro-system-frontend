...
import { useGetDistAdvanceReportQuery } from 'store/api/distReport/distReportApi';
import PrintDistAdvReceived from './PrintDistAdvReceived';
import DistAdvReceivedRow from './DistAdvReceivedRow';

const DistAdvReceived = () => {
  ...
  const { data, isLoading } = useGetDistAdvanceReportQuery(
    { ...query },
    {
      refetchOnMountOrArgChange: true,
    }
          ...
          <TableBody>
            {allAdvanceReports?.length ? (
              allAdvanceReports
                ...
                .map((item) => (
                  <DistAdvReceivedRow key={item.id} sn={sn++} data={item} />
                ))
            ) : (
              <TableRow>
                <StyledTableCellWithBorder
...
export default DistAdvReceived;
