...
import { useUpdateDrVoucherMutation } from 'store/api/drVoucher/drVoucherApi';
...
const UpdateDrVoucher = ({ open, handleClose, preData, allCustomers }) => {
  const { customer: preCustomer, drVoucherDetails, ...othersData } = preData;
  // library
  const findCurrentCustomer = allCustomers.find(
    (el) => el.id === preCustomer?.id
  );

  const voucheredInvoice = drVoucherDetails?.map((el) => ({
    ...el.invoice,
  ...
  const combinedInvoice = uniqueArray(
    voucheredInvoice || [],
    findCurrentCustomer?.drInvoices || [],
    'id'
  );
  // end library
  ...
  const [updateDrVoucher] = useUpdateDrVoucherMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await updateDrVoucher({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
...
export default UpdateDrVoucher;
