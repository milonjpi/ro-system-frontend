...
import { useCreateFosInvoiceMutation } from 'store/api/fosInvoice/fosInvoiceApi';
...
const AddFocInvoice = ({ open, handleClose, allCustomers }) => {
  ...
  const [createFosInvoice] = useCreateFosInvoiceMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await createFosInvoice({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        setCustomer(null);
