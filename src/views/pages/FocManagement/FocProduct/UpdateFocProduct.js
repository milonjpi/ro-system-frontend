...
import { useUpdateFosProductMutation } from 'store/api/fosProduct/fosProductApi';
...
const UpdateFocProduct = ({ open, handleClose, preData }) => {
  ...
  const [updateFosProduct] = useUpdateFosProductMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await updateFosProduct({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
...
export default UpdateFocProduct;
