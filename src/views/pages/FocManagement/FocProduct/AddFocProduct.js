...
import { useCreateFosProductMutation } from 'store/api/fosProduct/fosProductApi';
...
const AddFocProduct = ({ open, handleClose }) => {
  ...
  const [createFosProduct] = useCreateFosProductMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await createFosProduct({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
