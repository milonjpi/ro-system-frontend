...
import AddIncomeHead from './AddIncomeHead';
import IncomeHeadRow from './IncomeHeadRow';
...
const IncomeHead = () => {
  ...
  return (
    ...
    >
      ...
      {/* popup items */}
      <AddIncomeHead open={open} handleClose={() => setOpen(false)} />
      {/* end popup items */}
      <Box sx={{ overflow: 'auto' }}>
        <Table sx={{ minWidth: 450 }}>
          ...
          <TableBody>
            {allHeads?.length ? (
              allHeads.map((item) => (
                <IncomeHeadRow key={item.id} sn={sn++} data={item} />
              ))
            ) : (
              <StyledTableRow>
