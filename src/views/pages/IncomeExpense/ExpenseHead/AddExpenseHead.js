...
import { useCreateIncomeExpenseHeadMutation } from 'store/api/incomeExpenseHead/incomeExpenseHeadApi';
...
const AddExpenseHead = ({ open, handleClose }) => {
  ...
  const [createIncomeExpenseHead] = useCreateIncomeExpenseHeadMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await createIncomeExpenseHead({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
