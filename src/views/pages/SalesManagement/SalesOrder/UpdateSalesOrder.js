...
const UpdateSalesOrder = ({ open, handleClose, preData }) => {
  const [loading, setLoading] = useState(false);
  const [customer, setCustomer] = useState(preData?.customer || null);
  const [orderDate, setOrderDate] = useState(preData?.date || null);
  const [deliveryDate, setDeliveryDate] = useState(
    preData?.deliveryDate || null
  );
  const [products, setProducts] = useState(preData?.orderedProducts || []);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <LoadingButton
                ...
                type="submit"
              >
                <span style={{ lineHeight: 1 }}>Update Order</span>
              </LoadingButton>
            </Grid>
          </Grid>
        </Box>
