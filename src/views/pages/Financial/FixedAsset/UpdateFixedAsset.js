...
import { useUpdateFixedAssetMutation } from 'store/api/fixedAsset/fixedAssetApi';
...
const UpdateFixedAsset = ({ open, handleClose, preData }) => {
  const [loading, setLoading] = useState(false);
  const [date, setDate] = useState(preData?.date);
  const [asset, setAsset] = useState(preData?.asset);
  ...
  const [updateFixedAsset] = useUpdateFixedAssetMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await updateFixedAsset({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
