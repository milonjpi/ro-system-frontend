...
import { useCreateFixedAssetMutation } from 'store/api/fixedAsset/fixedAssetApi';
...
const AddFixedAsset = ({ open, handleClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await createFixedAsset({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
