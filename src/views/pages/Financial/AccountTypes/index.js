...
const AccountTypes = () => {
  ...
  const { data, isLoading } = useGetAccountTypesQuery(
    { ...query },
    { refetchOnMountOrArgChange: true }
  );

  const allAccountTypes = data?.accountTypes || [];
  ...
  return (
    ...
    >
      ...
      {/* popup items */}
      <AddAccountType open={open} handleClose={() => setOpen(false)} />
      {/* end popup items */}
          ...
          <TableBody>
            {allAccountTypes?.length ? (
              allAccountTypes.map((item) => (
                <AccountTypeRow key={item.id} sn={sn++} data={item} />
              ))
            ) : (
              <StyledTableRow>
