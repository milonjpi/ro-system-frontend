...
import { useCreateEquipmentMutation } from 'store/api/equipment/equipmentApi';
...
const AddItemName = ({ open, handleClose }) => {
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const res = await createEquipment({ ...data, isAsset: false }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
