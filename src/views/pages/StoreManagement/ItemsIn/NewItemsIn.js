...
import { useCreateEquipmentInMutation } from 'store/api/equipmentIn/equipmentInApi';
...
const NewItemsIn = ({ open, handleClose }) => {
  ...
  const [createEquipmentIn] = useCreateEquipmentInMutation();
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await createEquipmentIn({ ...newData }).unwrap();
      if (res.success) {
        handleClose();
        setLoading(false);
        reset();
