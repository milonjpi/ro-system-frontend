...
const UpdatePurchaseBill = ({ open, handleClose, preData }) => {
  const [loading, setLoading] = useState(false);
  const [vendor, setVendor] = useState(preData?.vendor || null);
  const [billDate, setBillDate] = useState(preData?.date);
  ...
  const { register, handleSubmit, control } = useForm({
    defaultValues: {
      refNo: preData?.refNo,
      equipments: preData?.billEquipments || [equipmentValue],
    },
  });
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <LoadingButton
                ...
                type="submit"
              >
                <span style={{ lineHeight: 1 }}>Update Bill</span>
              </LoadingButton>
            </Grid>
          </Grid>
        </Box>
...
export default UpdatePurchaseBill;
