...
import { useUpdateJewelleryRateMutation } from 'store/api/jewelleryRate/jewelleryRateApi';
...
const UpdateRate = ({ open, handleClose, preData, category }) => {
  ...
  const [updateJewelleryRate] = useUpdateJewelleryRateMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await updateJewelleryRate({
        id: preData?.id,
        body: newData,
      }).unwrap();
      if (res.success) {
