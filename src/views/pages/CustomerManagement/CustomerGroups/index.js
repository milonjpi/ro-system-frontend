...
const CustomerGroups = () => {
  ...
  const { data, isLoading } = useGetCustomerGroupsQuery(
    { ...query },
    { refetchOnMountOrArgChange: true }
  );

  const allCustomerGroups = data?.customerGroups || [];
  ...
  return (
    ...
    >
      ...
      {/* popup items */}
      <AddCustomerGroup open={open} handleClose={() => setOpen(false)} />
      {/* end popup items */}
        ...
        <Table sx={{ minWidth: 450 }}>
          ...
          <TableBody>
            {allCustomerGroups?.length ? (
              allCustomerGroups.map((item) => (
                <CustomerGroupRow key={item.id} sn={sn++} data={item} />
              ))
            ) : (
              <StyledTableRow>
